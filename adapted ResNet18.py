...
class CustomDataset(Dataset):
    ...
    def load_images(self, image_paths):
        ...
        for img_path in image_paths:
            ...
            # Crop the image
            cropped_read_image = read_image[:-bottom_crop, int(self.crop_left * width):width - int(self.crop_right * width)]
        ...
        # Stack exam_images along the batch dimension to create a tensor of shape [num_images_per_exam, height, width]
...
class ResNet18(nn.Module):
    def __init__(self):
        ...
        # Change the first convolutional layer to accept 16-channel input
        self.resnet.conv1 = nn.Conv2d(16, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)

        # Get the number of features for the fully connected layer
        num_features = self.resnet.fc.in_features

        # Modify the fully connected layer for single output
        self.resnet.fc = nn.Linear(num_features, 1)

        # Additional fully connected layer to process the combined features
        self.final_fc = nn.Linear(3, 1)  # 1 feature from ResNet + 2 for the eye parameters

    def forward(self, x, eye):
        batch_size = x.size(0)

        # Process the image through the resnet model
        x = self.resnet(x)

        # Ensure the eye tensor is the right shape
        eye = eye.view(batch_size, -1)  # Reshape to [batch_size, 2]

        # Concatenate the eye parameter with the output from ResNet
...
for epoch in range(num_epochs):
    ...
    model_save_path = rf'C:\Maziar Mirsalehi\Python codes\CNNs\ResNet18 model of epoch\ResNet18_Real ESI Real Image_{epoch + 1}_{current_date}.pth'
...
predictions_df.to_csv(rf'C:\Maziar Mirsalehi\Results\ResNet18\ResNet18_Test predictions_{current_date}.csv', index=False)
...
current_date = datetime.datetime.now().strftime("%d.%m.%Y")
mse_df.to_csv(rf'C:\Maziar Mirsalehi\Results\ResNet18\ResNet18_{current_date}.csv', index=False)

#######################################################################################################################################

# Save the entire model state dictionary
current_date = datetime.datetime.now().strftime("%d.%m.%Y")
torch.save(model, rf'C:\Maziar Mirsalehi\Python codes\CNNs\ResNet18 model\Resnet18_{current_date}.pth')
